...
public:
  ...
  void writeSpumuxXml(const QString &spumuxFile, const QString &subFile) const
  {
    ...
    textsub.setAttribute("horizontal-alignment", slideshow.subtitleSettings().horizontalAlign());
...
public:
  ...
  void run()
  {
    ...
    if(writeSlideshowFile())
    {
      ...
      *dvdslideshow << app;
      ...
      {
        QString format=QLatin1String("dv_")+QLatin1String("DVD-PAL"==projectType ? "pal" : "ntsc")+
                       QLatin1String(wideScreen ? "_wide" : "");
        *dvdslideshow << "-progress"
                      ...
                      << "f=dvd"
                      << "vcodec=mpeg2video"
                      << QLatin1String(SlideshowPluginSettings::audioType() == 0 ? "acodec=mp2" : "acodec=ac3")
                      << "b=5000k"
                      << "maxrate=8000k"
                      << "minrate=0"
                      << "bufsize=1835008"
        ...
        setFilter(QLatin1String("^\\s*Current Frame:\\s+\\d+,\\s+percentage:\\s+\\d+\\s*$"));
      }
      else
      {
  ...
  {
    QDir dir(mediaDir);
    QString output = dir.filePath(QString(SlideshowPlugin::BACKEND_MELT==backend ? "%1.mlt"
       ...
       subFile.open(QIODevice::WriteOnly | QIODevice::Truncate)))
    {
      ...
      if(SlideshowPlugin::BACKEND_MELT==backend)
      {
          ...
          double fps="DVD-PAL"==projectType ? 25.0 : (30000.0/1001.0),
                 totalDuration=(slideshow.slides().count()*duration)+constPictureFade;
          ...
          ts << "<?xml version=\'1.0\' encoding=\'utf-8\'?>" << endl
             << "<!--" << endl
             << "This file was made with " << KGlobal::mainComponent().aboutData()->programName()
                << " - " << KGlobal::mainComponent().aboutData()->version() << endl
             << "http://www.iki.fi/damu/software/kmediafactory/" << endl
             << "-->" << endl
             << "<mlt title=\"Anonymous Submission\" root=\"" << mediaDir << "\" >" << endl;

          // Add producer that creates black background
          ts << "  <producer in=\"0\" out=\"" << endFrame(totalFrames) << "\" id=\"black\" >" << endl
             ...
             << "    <entry in=\"0\" out=\"" << endFrame(totalFrames) << "\" producer=\"black\" />" << endl
             << "  </playlist>" << endl;
          ...
          if(slideshow.audioFiles().count())
          {
            ...
            foreach(const QString &audio, slideshow.audioFiles())
            {
                ...
                files << "  <producer in=\"0\" out=\"" << endFrame(useFrames) << "\" id=\"a_" << file << "\" >" << endl
                      << "    <property name=\"mlt_type\" >producer</property>" << endl
                      << "    <property name=\"aspect_ratio\" >0.000000</property>" << endl
                      << "    <property name=\"resource\" >" << audio << "</property>" << endl
...
//                            << "      </filter>" << endl;
          ...
          // Pictures...
          //     Pics 1, 3, 5, etc are placed on one track, and 2, 4, 6, etc on another...
          int pic=0;
          QString track1Pics, track2Pics, track1Playlist, track2Playlist;
          QTextStream t1pics(&track1Pics), t2pics(&track2Pics), t1playlist(&track1Playlist), t2playlist(&track2Playlist),
                      sub(&subFile);
          ...
          foreach(Slide slide, slideshow.slides())
          {
              ...
              bool        first=0==pic, last=(slideshow.slides().count()-1)==pic;
              ...
              pics << "  <producer in=\"0\" out=\"" << endFrame(pictureFrames+pictureFadeFrames) << "\" id=\"" << pic+1 << "\" >" << endl
                   << "    <property name=\"mlt_type\" >producer</property>" << endl
                   << "    <property name=\"aspect_ratio\" >1</property>" << endl
                   << "    <property name=\"resource\" >" << slide.picture << "</property>" << endl
                   << "    <property name=\"progressive\" >1</property>" << endl
                   << "    <property name=\"mlt_service\" >pixbuf</property>" << endl
                   << "  </producer>" << endl;

              currentList << "    <entry in=\"0\" out=\"" << endFrame(pictureFrames+pictureFadeFrames)
                                  << "\" producer=\"" << pic+1 << "\" >" << endl;
              if(first) // Fade in from black...
                  currentList << "       <filter in=\"0\" out=\"" << endFrame(pictureFadeFrames) << "\" id=\"fade_from_black\" >" << endl
                              << "         <property name=\"track\" >0</property>" << endl
                              << "         <property name=\"start\" >0</property>" << endl
                              << "         <property name=\"mlt_type\" >filter</property>" << endl
                              << "         <property name=\"mlt_service\" >brightness</property>" << endl
                              << "         <property name=\"tag\" >brightness</property>" << endl
                              << "         <property name=\"end\" >1</property>" << endl
                              << "       </filter>" << endl;
              if(last) // Fade out to black...
                  currentList << "       <filter in=\"" << pictureFrames
                                         << " \" out=\"" << endFrame(pictureFrames+pictureFadeFrames) << "\" id=\"fade_to_black\" >" << endl
                              << "         <property name=\"track\" >0</property>" << endl
              ...
              otherList   << "    <blank length=\"" << (first ? pictureFrames : pictureFrames-pictureFadeFrames) << "\" /> " << endl;
              pic++;
          }
          ...
          ts << "  <tractor title=\"Anonymous Submission\" global_feed=\"1\" in=\"0\" out=\""
          ...
          for(int i=1; i<slideshow.slides().count(); ++i)
              ts << "    <transition in=\"" << (i*pictureFrames) << "\" out=\"" << endFrame((i*pictureFrames)+pictureFadeFrames)
                 ...
                 << "      <property name=\"reverse\" >" << (i%2 ? 1 : 0) << "</property>" << endl
                 << "    </transition>" << endl;
          ts << "  </tractor>" << endl
             << "</mlt>" << endl;
      }
...
QVariant SlideshowObject::writeDvdAuthorXml(QVariantList args) const
{
  ...
  QString subLang = SlideshowPlugin::BACKEND_MELT==static_cast<SlideshowPlugin*>(plugin())->backend()
