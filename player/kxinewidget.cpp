/***************************************************************************
    ...
    copyright            : (C) 2003-2005 by JÃŒrgen Kofler
...
KXineWidget::KXineWidget(QWidget* parent,
		...
		: QWidget(parent), QThread(), m_startXineManual(startManual), m_xineReady(false),
...
bool KXineWidget::playDvb()
{
	...
	Q3PtrList<PostFilter> activeList;
...
bool KXineWidget::openDvb()
{
	...
	if (!xine_open(m_xineStream, m_trackURL.ascii())) {
		sendXineError();
		return false;
	}
...
void KXineWidget::slotPlayTimeShift()
{
	...
	if (!xine_open(m_xineStream, TimeShiftFilename.ascii())) {
		sendXineError();
#ifdef XINE_PARAM_GAPLESS_SWITCH
		if ( xine_check_version(1,1,1) )
...
QString KXineWidget::getXineLog() const
{
	QString logStr;
	int i = 0;
	Q3TextStream ts(&logStr, QIODevice::WriteOnly);
...
#if XINE_MAJOR_VERSION >= 1 && XINE_MINOR_VERSION >= 1
