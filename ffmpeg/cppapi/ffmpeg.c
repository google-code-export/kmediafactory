...
#if !defined CONFIG_WIN32 && !defined FFMPEG_MAIN
...
int av_transcode()
{
  ...
  if (nb_output_files <= 0)
  {
    av_log(NULL, AV_LOG_INFO, "Must supply at least one output file\n");
    return 1;
  }

  if (nb_input_files == 0)
  {
    av_log(NULL, AV_LOG_INFO, "Must supply at least one input file\n");
    return 1;
  }

  result = av_encode(output_files, nb_output_files, input_files, nb_input_files,
                     stream_maps, nb_stream_maps);
...
void av_reset()
{
  ...
  video_mb_lmin = 2*FF_QP2LAMBDA;
  video_mb_lmax = 31*FF_QP2LAMBDA;
  video_qdiff = 3;
  video_qblur = 0.5;
  video_qsquish = 0.0;
  video_qcomp = 0.5;
...
void av_set(const char* opt, const char* arg)
{
  int optindex;
...
{
  received_sigterm = stop;
}

void set_progress(Progress* progress)
{
  av_progress = progress;
}
